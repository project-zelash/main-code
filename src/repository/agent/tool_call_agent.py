...
class ToolCallAgent(BaseAgent):
    ...
    def run(self, user_query):
        ...
        try:
            ...
            if self.verbose:
                logger.info(f"🎯 {self.name} received query: {user_query}")
            self.current_iteration = 0
            final_response = ""
            retries = 0
            last_prompt = None
            while self.current_iteration < self.max_iterations:
                ...
                try:
                    ...
                    if tool_code:
                        ...
                        if bash_command:
                            if self.verbose:
                                logger.info(f"🔧 Fallback: Executing bash command from code block: {bash_command}")
                    ...
                    if tool_calls:
                        if self.verbose:
                            logger.info(f"🛠️ {self.name} wants to use {len(tool_calls)} tools")
                        # Execute tool calls
            ...
            if self.verbose:
                logger.info(f"✅ {self.name} completed in {self.current_iteration} iterations")
    ...
    def _process_tool_calls(self, tool_calls):
        ...
        for tool_call in tool_calls:
            try:
                ...
                if self.verbose:
                    logger.info(f"✅ Tool '{tool_name}' completed. Result: {str(result)[:100]}...")
                
                results.append(result)
                
            except Exception as e:
                error_result = f"Error executing tool '{tool_name}': {str(e)}"
                logger.error(f"🚨 {error_result}")
                results.append(error_result)
